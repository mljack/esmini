...
3. **E3 持久化与列表**：schema v2（关键帧 $(t, s)$）、Keyframes 表格/状态列、Go/Delete、speed 图表 $s^*_k$ 参考线联动、路径几何变动后的自动钳制+重解挂钩。
...
**剩余待确认项**（不阻塞 E1 开工，可在实现过程中逐个确认）：

1. **加速度检查的范围**：只检查本次级联修改波及的分段（建议），还是顺带全表扫描、把用户手工编辑造成的既有超限段也标出来（可做成仅警告显示、不拦截）？
2. **钳制兜底的确认**：P0/P3 无解时"钳制到最近可行值 + ⚠ 残差落库 + 提示"的兜底语义（用户对 P3 的回答已隐含认可钳制+提示，这里仅作最后确认）。
3. **$a_{acc}/a_{dec}$ 是否需要在 UI 中可配置**（如 Esmini Settings 弹窗里），还是代码常量即可（第一期建议常量）？
