...
## 12. 求解器优化方案 v2（已实现，见 12.7）：可达范围预钳制 + 最小加点原则
...
按 12.6 的顺序 ①-④ 全部实现并逐步提交。为满足"求解器可脱离 UI 无头测试"的要求，速度曲线与求解器整体抽取为自包含模块 `TrajectorySolver.hpp/.cpp`（仅依赖标准库，`EntitySpeedProfile`/`TrajectoryKeyframe`/可达性 oracle/`SolveKeyframeChain`/`RemoveKeyframe`），`StudioDataModel` 中的 `EntityTrajectory::ResolveKeyframes()/RemoveKeyframe()` 只做委托；限值收进 `TrajectorySolverLimits` 结构（默认值即第 11 节定稿参数），便于测试注入其他值。单元测试 `StudioTrajectorySolver_test`（31 项，直接编译求解器源文件、不链接 OSG）覆盖：曲线插值/积分/反查、oracle 时间窗与最远可达距离的单调性与一致性、Q1 两族（乘法保形/静止起步加法）、Q0/Q3 物理钳制不插点、Q2 恰插 1 点、pin 生命周期（重复求解不累积/拖动迁移/删除清理/锚点与共享保护）、边界值（s=0、t=0、越界钳制、重复 t、空输入）与组合操作（路径缩短、手工节点保留、链式钳制传递）。

实现与设计稿的差异（多数由测试暴露后修正）：

1. **首段 Q0 窗口按自由入速计算**：v(0) 本就可调（第 11 节决策），首段的物理窗口取恒速极值 $[L/v_{max},\ L/v_{min}]$，而不是按当前 v(0) 算——否则静止起步（v(0)=0）会被误判不可达而钳制。
2. **5 秒合并增加"残差回退则撤销"守护**：合并除不得破坏加速度限制外，若使残差明显恶化（超过求解容差）也撤销。分析表明合并到边界后的形状等价于 Q1a 族成员，Q2 采纳判据必然回退到 Q1，因此间距规则实际让位于精度，与其"软规则"定位一致。
3. **12.5 的"平台点并入/吸附"简化**：Q2 构造中平台速度由 pin 节点承担，段内本就至多 1 个坡道点，合并即直接删除该点后重验，无需三角形化/吸附分支。
4. **路径缩短的清理**：链式求解入口处删除超出新总长的陈旧 profile 节点（保持有序性），配合关键帧 s 钳制完成自动重解。
5. **拖动预钳制的后向界**：下一关键帧的可覆盖性用乐观界 $s \ge s_{next} - v_{max}\cdot(t_{next}-t)$（入速未知，取上限恒速），前向界用共享 oracle 的 `MaxReachableDistance`（上一 pin 的当前入速；首段取 $v_{max}$）。可达范围以绿色加粗子线段叠加在路径线上，拖动结束/实体删除/重载轨迹时清除。
