...
namespace
{
    float g_scroll_scale     = 0.2f;
    float mouse_prev_x       = 0;
    float mouse_prev_y       = 0;
    bool  mouse_prev_xy_init = false;
};  // namespace
...
bool TopViewManipulator::handle(const GUIEventAdapter& ea, GUIActionAdapter& us)
{
    if (ea.getEventType() & (GUIEventAdapter::MOVE | GUIEventAdapter::PUSH | GUIEventAdapter::RELEASE))
    {
        ...
        double x_offset = tan * camera_z * ea.getXnormalized() * ea.getWindowWidth() / ea.getWindowHeight();
        double y_offset = tan * camera_z * ea.getYnormalized();
    ...
    if (ea.getEventType() & GUIEventAdapter::PUSH)
    {
        ...
        {
            mouse_prev_x = ea.getXnormalized();
            mouse_prev_y = ea.getYnormalized();
        }
    }
    ...
    switch (ea.getEventType())
    {
        case (GUIEventAdapter::DRAG):
            if (ea.getButtonMask() == GUIEventAdapter::RIGHT_MOUSE_BUTTON)
            {
                if (mouse_prev_xy_init)
                {
                    double tan      = std::tan(M_PI / 360.0);
                    double camera_z = cameraPos_[2] - cameraTargetPos_[2];
                    double dx       = tan * camera_z * (ea.getXnormalized() - mouse_prev_x) * ea.getWindowWidth() / ea.getWindowHeight();
                    double dy       = tan * camera_z * (ea.getYnormalized() - mouse_prev_y);

                    if (bounding_sphere_.contains(osg::Vec3d(cameraPos_[0] + dx, cameraPos_[1] + dy, 0.0)))
                    {
                        cameraPos_[0] += dx;
                        cameraPos_[1] += dy;
        ...
        case (GUIEventAdapter::SCROLL):
        {
            ...
            _cameraDistance -= (static_cast<float>(scroll)) * _cameraDistance * g_scroll_scale;
            _cameraDistance = std::min(300000.0, std::max(500.0, _cameraDistance));
            ...
            if (bounding_sphere_.contains(osg::Vec3d(mousePos_[0] - dx, mousePos_[1] - dy, 0.0)))
            {
...
bool TopViewManipulator::moveCameraTo(double x, double y)
{
    ...
    double tan      = std::tan(M_PI / 360.0);
    double camera_z = cameraPos_[2] - cameraTargetPos_[2];
    double x_offset = 2 * viewport_offset_x_ / viewport_height_ * tan * camera_z;
    double y_offset = 2 * viewport_offset_y_ / viewport_height_ * tan * camera_z;
    x += x_offset;
    y += y_offset;

    cameraPos_[0]       = x;
    cameraPos_[1]       = y;
    cameraTargetPos_[0] = x;
    cameraTargetPos_[1] = y;
    return true;
}
...
ViewSetting TopViewManipulator::getViewSetting()
{
    return {cameraPos_[0], cameraPos_[1], cameraPos_[2], cameraTargetPos_[0], cameraTargetPos_[1], cameraTargetPos_[2], _cameraDistance};
}

void TopViewManipulator::setViewSetting(const ViewSetting& settings)
{
    cameraPos_[0]       = settings.camera_pos_x_;
    cameraPos_[1]       = settings.camera_pos_y_;
    cameraPos_[2]       = settings.camera_pos_z_;
    cameraTargetPos_[0] = settings.camera_target_pos_x_;
    cameraTargetPos_[1] = settings.camera_target_pos_y_;
    cameraTargetPos_[2] = settings.camera_target_pos_z_;
    _cameraDistance     = settings.camera_distance_;
}

void TopViewManipulator::backupViewSetting()
{
    view_settings_.push_back(getViewSetting());
}

void TopViewManipulator::restoreViewSetting()
{
    if (!view_settings_.empty())
    {
        setViewSetting(view_settings_.back());
        view_settings_.pop_back();
    }
}

void TopViewManipulator::setBound(const osg::BoundingSphere& bounding_sphere)
{
    bounding_sphere_ = bounding_sphere;
}

void TopViewManipulator::setViewportSize(double width, double height)
{
    viewport_width_  = width;
    viewport_height_ = height;
}

void TopViewManipulator::setViewportCenterPixelOffset(double pix_x, double pix_y)
{
    viewport_offset_x_ = pix_x;
    viewport_offset_y_ = pix_y;
}

void TopViewManipulator::MoveMouse(GUIActionAdapter& aa, double x, double y)
{
    if (viewport_width_ <= 0 || viewport_height_ <= 0)
        return;

    double tan_val  = std::tan(M_PI / 360.0);
    double camera_z = cameraPos_[2] - cameraTargetPos_[2];

    double pix_x = (x - cameraPos_[0]) / (tan_val * camera_z) * viewport_height_ / 2;
    double pix_y = (y - cameraPos_[1]) / (tan_val * camera_z) * viewport_height_ / 2;
    pix_x        = viewport_width_ / 2 - pix_x;
    pix_y        = viewport_height_ / 2 - pix_y;

    osg::View* view = dynamic_cast<osg::View*>(&aa);
    if (view && view->getCamera() && view->getCamera()->getViewport())
    {
        pix_x += view->getCamera()->getViewport()->x();
        pix_y += view->getCamera()->getViewport()->y();
    }

    aa.requestWarpPointer(pix_x, pix_y);
}
